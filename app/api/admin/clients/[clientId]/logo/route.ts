...
export async function POST(request: Request, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  const path = `${clientId}/logo/${Date.now()}-logo.${image.extension}`;
  const { error: uploadError } = await context.supabase.storage
    .from("client-assets")
    .upload(path, await image.file.arrayBuffer(), { contentType: image.file.type, upsert: false });
  if (uploadError) return NextResponse.json({ error: "Unable to upload client logo." }, { status: 500 });

  const { error } = await context.supabase
    .from("clients")
    .update({ logo_path: path, updated_by: context.profile.id })
    .eq("id", clientId);
  if (error) return NextResponse.json({ error: "Logo uploaded but client could not be updated." }, { status: 500 });

  await logAuditEvent(context, "client_logo_updated", "client", clientId);
