...
import { validateAvatarFile, avatarPublicUrl, isDemoPresentationOwnerEmail } from "@/lib/profile/avatar";
...
export async function GET(request: Request) {
  ...
  if (isDemoPresentationOwnerEmail(context.profile.email)) {
...
export async function POST(request: Request) {
  ...
  if (isDemoPresentationOwnerEmail(context.profile.email)) {
    return NextResponse.json(
      { error: "El avatar permanente de esta cuenta demo es U." },
      { status: 409 }
    );
  }
