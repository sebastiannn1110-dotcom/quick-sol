...
export default function SourcingWorkspace() {
  ...
  async function automate(event: FormEvent) {
    event.preventDefault();
    const ok = await mutate("/api/sourcing/automation/request", { commerceRfqItemId: rfqItemId }, c.automationDone);
    if (ok) setRfqItemId("");
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <form onSubmit={automate} className="space-y-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="font-semibold text-slate-950">{c.automation}</h2>
          <p className="text-sm text-slate-600">{c.automationHelp}</p>
          <input required className={`${inputClass} w-full`} placeholder={c.rfqItem} value={rfqItemId} onChange={(event) => setRfqItemId(event.target.value)} />
          <button disabled={busy} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{c.sendSourcing}</button>
        </form>
      </div>
