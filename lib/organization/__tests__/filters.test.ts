...
const MEMBERS: OrganizationMember[] = [
  member("demo-owner", null, "Demo Owner — DEMO", "Chief Executive Officer — DEMO", "owner", "Executive — DEMO", "Singapore"),
  member("olivia", "demo-owner", "Olivia Mercer — DEMO", "Chief Operating Officer — DEMO", "executive", "Executive — DEMO", "United States"),
...
describe("Team Structure filters", () => {
  ...
  it("derives sorted options only from members already visible to the actor", () => {
    ...
    expect(options.teamManagers.map((item) => item.profileId)).toEqual(["daniel", "demo-owner", "lin", "olivia"]);
  });
