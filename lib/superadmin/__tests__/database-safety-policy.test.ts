...
describe("Database Safety Center policy", () => {
  ...
  it("uses an explicit 50-table DELETE allowlist and preserves identity, security and observability", () => {
    ...
    expect(DATABASE_SAFETY_DELETE_TABLES.map((entry) => `${entry.schema}.${entry.table}`)).toEqual(expect.arrayContaining([
      "public.business_summary_mpn_stage", "public.business_summary_entity_stage"
    ]));
  ...
  it("preserves private sourcing assets and keeps rebranded destruction confirmation", () => {
