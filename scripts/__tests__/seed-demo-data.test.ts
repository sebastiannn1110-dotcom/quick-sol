...
describe("DEMO data manifest", () => {
  it("is internally consistent and restricts the one non-reserved login to Jason", () => {
    expect(validateDemoManifest()).toBe(DEMO_DATA_MANIFEST);
    expect(DEMO_DATA_MANIFEST.people).toHaveLength(28);
    expect(DEMO_DATA_MANIFEST.people.filter((person) => !person.email.endsWith(".demo.invalid"))).toEqual([
      expect.objectContaining({ key: "jason", email: "jasonBoss@quiksol.com" })
    ]);
  ...
  it("uses only fixed technical roles and makes Jason the sole root and owner", () => {
    expect(new Set(DEMO_DATA_MANIFEST.people.map((person) => person.technicalRole))).toEqual(
      new Set(["admin", "manager", "employee"])
    );
    expect(DEMO_DATA_MANIFEST.people.find((person) => person.key === "jason")).toMatchObject({
      fullName: "Jason Boss \u2014 DEMO",
      technicalRole: "admin",
  ...
  it("orders the hierarchy with Jason first and every manager before direct reports", () => {
    const ordered = demoPeopleInHierarchyOrder();
    const position = new Map(ordered.map((person, index) => [person.key, index]));
    expect(ordered[0]?.key).toBe("jason");
...
describe("DEMO seed R8 provisioning flow", () => {
  ...
  it("bootstraps only Olivia through CLI and provisions all others through her admin session", async () => {
    ...
    expect(calls.admin.find((person) => person.key === "jason")?.technicalRole).toBe("admin");
    expect(calls.create.map(({ person }) => person.key)).toEqual(DEMO_DATA_MANIFEST.people.map((person) => person.key));
    expect(calls.create.find(({ person }) => person.key === "jason")?.password).toBe(ownerPassword);
    expect(calls.create.filter(({ person, password }) => person.key !== "jason" && password === strongPassword)).toHaveLength(27);
    expect(calls.authenticate).toEqual([{ person: olivia, expectedUserId: personIds.olivia }]);
    expect(calls.verifyOwner).toEqual([{ person: team.find((person) => person.key === "jason"), password: ownerPassword, expectedUserId: personIds.jason }]);
    expect(calls.sequence.indexOf("authenticate:olivia")).toBeLessThan(calls.sequence.indexOf("verify-owner:jason"));
    expect(calls.sequence.indexOf("verify-owner:jason")).toBeLessThan(calls.sequence.indexOf("profile:olivia"));
  ...
  it("is a no-create no-begin operation on the second run, including mixed-case Jason email", async () => {
...
describe("DEMO seed source boundary", () => {
  ...
  it("does not touch Opportunity Finder, revenue, or sales tables", () => {
