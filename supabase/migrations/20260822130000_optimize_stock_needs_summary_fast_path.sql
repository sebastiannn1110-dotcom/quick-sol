-- Keep Stock Needs inside the authenticated statement timeout at production
-- summary volume. Authorization is evaluated once per visible upload, while
-- source JSON is materialized only for the requested page.
create index if not exists business_mpn_summaries_stock_rollup_idx
